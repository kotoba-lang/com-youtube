...
(defn insert-video!
  ...
  ([access-token video-bytes metadata {:keys [http-fn] :or {http-fn (client/jvm-http-fn)}}]
   (let [init-resp (http-fn {:url videos-insert-url
                             :method :post
                             :headers (merge (client/auth-header access-token)
                                            {"Content-Type" "application/json; charset=UTF-8"
                                             "X-Upload-Content-Type" "video/mp4"
                                             "X-Upload-Content-Length" (str (alength video-bytes))})
     ...
     (let [upload-url (get (:response-headers init-resp) "location")]
       ...
       (let [put-resp (http-fn {:url upload-url
                                ...
                                :headers {"Content-Type" "video/mp4"
                                         "Content-Length" (str (alength video-bytes))}
                                :body video-bytes})]
...
(defn update-video!
  ...
  ([access-token video-id metadata {:keys [http-fn] :or {http-fn (client/jvm-http-fn)}}]
